...
def parse_input(path, times):
    with open(path) as file:
        spring_sets = []

        for line in [x.strip().split() for x in file.readlines()]:
            springs = "?".join([line[0]] * times)
            groups = tuple(int(x) for x in re.findall(r"\d+", line[1])) * times
...
def get_valid_permutations(springs, groups):
    ...
    key = (springs, groups)
    if key in valid_permutations_cache:
        return valid_permutations_cache[key]
    ...
    valid_permutations_cache[key] = sum_valid_permutations
    return sum_valid_permutations


spring_sets = parse_input("input.txt", 5)
